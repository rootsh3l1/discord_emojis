...
var FreeEmojis = (() => {
    ...
    function Start() {
        ...
        // Spoof Nitro to enable high-quality screen sharing
        let tries = 1;
        let intervalId = setInterval(() => {
            if (++tries > 5) clearInterval(intervalId);
    
            const user = DiscordModules.UserStore.getCurrentUser();
            if (!user) return;
            user.premiumType = 2;
    ...
    return function () {
        return {
            getName: () => "Discord emoji + nitro bypass",
            getShortName: () => "ROOTMojis",
            getDescription: () => "Free emojis and nitro bypass.",
            getVersion: () => "1.0.0",
            getAuthor: () => "il tuo amichetto root :)",
